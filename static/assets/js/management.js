	...
	if(regsFormValidate()){
		...
		$.ajax({
			...
			success: function(respData, textStatus, jqXHR) {
				...
				// 告知发送邮件
				window.location.href = '/regist_org_success?mailSuffix='+mailSuffix+'&regEmail='+regEmail;
			},
			...
			$.ajax({
				...
				success: function(respData, textStatus, jqXHR) {
				  ...
				  $.cookie('appName',app,{path:'/',expires:date});
					...
					window.location.href = '/app_profile';
				}
		});
	}
}
			...
			$.ajax({
				...
				success: function(respData, textStatus, jqXHR) {
					...
					$(respData.entities).each(function(){
						window.location.href = '/app_profile?appUuid=' + this.uuid;
					});
				}
			});
		}
		...
		$.ajax({
			...
			success: function(respData, textStatus, jqXHR) {
				...
				$.each(appData,function(key,value){
					...
					option += '<tr><td class="text-center"><a href="/app_profile?appUuid='+value+'&Application='+key+'">'+key+'</a></td>'+
...
function togroupaddAppAdminuserusers(appUuid,groupid){
	window.location.href = '/app_qunzu_list?appUuid=' + appUuid + '&groupid=' + groupid;
}
...
function deleteAppChatroom(appUuid,groupuuid){
	...
	if(confirm('确定要删除此群组吗?')){
		$.ajax({
			...
			success:function(respData){
				alert('提示\n\n删除成功!');
				window.location.href = '/app_group?appUuid='+appUuid;
			}
		});
	}
}
...
function toAppIMList(owner_username){
	window.location.href = '/app_IM_list?appUuid='+appUuid+'&owner_username='+owner_username;
}
//获取用户好友列表
